...
def company_achievability(plan_dict):
    """
    Achievability % = (sum of reachable_i) / (sum of target_i) * 100
    where:
        reachable_i = min(model suggestion, entered target) for each salesperson
    """
    if not plan_dict:
        return 0.0, 0.0, 0.0
    ...
    for code, rec in plan_dict.items():
        ...
        if not np.isnan(suggested):
            # Each salesperson: achievable is the LOWER of target vs model suggestion
            reachable_total += min(float(suggested), float(rec.get("target", 0.0)))

    pct = (reachable_total / entered_total) * 100.0
    return pct, reachable_total, entered_total

def render_achievability(ach_pct: float, reachable_amt: float, entered_total: float):
    """Show company-wide achievability %."""
    st.metric(
        "Achievability %",
        f"{ach_pct:.1f}%",
        f"{reachable_amt:,.0f} reachable out of {entered_total:,.0f} target"
    )
    st.progress(min(100, int(round(ach_pct))))
...
with left:
    ...
    with tab_existing:
        ...
        # Ensure rep_select exists in session_state
        ...
        if picker_options:
            # Use index to keep the right item selected
            current_index = (
                picker_options.index(st.session_state.rep_select)
                if st.session_state.rep_select in picker_options else 0
            )
        ...
        if sp_code:
            ...
            if "reset_name_flag" in st.session_state:
                # clear widget value before creating the input
                st.session_state.pop("edit_name", None)
                del st.session_state["reset_name_flag"]
                name_prefill = ""  # always reset to blank
            else:
                name_prefill = current_plan.get("name", st.session_state.name_map.get(sp_code, ""))

            edited_name = st.text_input(
                "Salesperson name (reference)",
                value=name_prefill,
                key="edit_name"
            )
            ...
            with st.form("entry_form_existing", clear_on_submit=False):
                ...
                if add_upd:
                    ...
                    # 🔑 Reset name field on next rerun
                    st.session_state["reset_name_flag"] = True
